    ...
    <div class="wrap">
        ...
        <form method="post" action="options.php">
            ...
            <table class="form-table">
                ...
                <tr valign="top">
                    <th scope="row">Table Width (e.g., 100%, 400px)</th>
                    <td><input type="text" name="prayer_times_text_options[table_width]" value="<?php echo esc_attr($display_options['table_width'] ?? '100%'); ?>" /></td>
                </tr>
            </table>
...
function display_prayer_times_table() {
    ...
    // Apply user-defined styles from settings
    $table_style = 'width: ' . esc_attr($display_options['table_width']) . '; border-collapse: collapse;';
    $font_style = 'font-family: ' . esc_attr($display_options['font_family']) . '; font-size: ' . esc_attr($display_options['font_size']) . 'px;';
    ...
    if (array_key_exists($current_date, $prayer_times)) {
        ...
        echo "<h2 style='$font_style'>Prayer Times</h2>";
        echo "<p style='$font_style'>for the Region of Waterloo</p>";
        echo "<p style='$font_style'>" . date('jS F Y') . "</p>";
        ...
        foreach ($times as $prayer => $time) {
            if (isset($options[$prayer]) && $options[$prayer]) {
                ...
                echo "<tr>";
                echo "<td style='padding: 8px; border: 1px solid #ddd; white-space: nowrap; $font_style $color_style'>$prayer</td>";
                echo "<td style='padding: 8px; border: 1px solid #ddd; white-space: nowrap; $font_style $color_style'>" . convert_to_12h_format($time) . "</td>";
                echo "</tr>";
            }
        }
